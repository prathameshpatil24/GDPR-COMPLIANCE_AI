...
**English-only runtime for v1–v3.** German-first multilingual retrieval and UI are **v4** scope (see [Documentation README](../README.md) and [v4-overview.md](../v4-overview.md)). **v4** prioritises **near-100% accuracy architecture** (deterministic mapping, verification, confidence scoring), then a **Retrieval Gap Tracker** (KB and map expansion loop); that does not change the English-only **response** policy until multilingual work ships.
...
* **Accessibility**: **v4** adds **near-100% accuracy** measures first, then German-first multilingual UI and retrieval, plus **retrieval gap** visibility to improve grounding and **article-map** curation over time (see [v4-overview.md](../v4-overview.md))
...
* **v4** roadmap leads with **near-100% accuracy architecture**, then **Retrieval Gap Tracker**, multilingual UI and retrieval (German-first), document upload, and website scanning — see [v4-roadmap.md](../v4-roadmap.md)