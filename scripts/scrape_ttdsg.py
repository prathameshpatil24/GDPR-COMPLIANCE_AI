#!/usr/bin/env python3
"""Scrape TTDSG sections from gesetze-im-internet.de (German text)."""
...
TTDSG_BASE = "https://www.gesetze-im-internet.de/ttdsg"
ROOT = Path(__file__).resolve().parents[1]
OUT_PATH = ROOT / "data" / "raw" / "ttdsg_sections.json"
...
async def discover_pages(client: httpx.AsyncClient) -> list[str]:
    resp = await client.get(f"{TTDSG_BASE}/", headers=DEFAULT_HEADERS, timeout=60.0)
...
async def main() -> None:
    ...
    logger.info("Wrote %s TTDSG sections to %s", len(sections), OUT_PATH)
